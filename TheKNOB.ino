...
int IRAM_ATTR key_detect(int enc_tick) {//remove whole keypresses from ISR and replace with buffer.
  ...
  pin_state_now |= (digitalRead(ENCODER_A) << 1);
  pin_state_now |= digitalRead(ENCODER_B);
  ...
  #define ENCODER_FLOP_COOLDOWN 750
  static int last_send = 0; //0 = non encoder, 1 = cw, 2 = ccw

  
  if(interrupt_time - last_interrupt_time > 20){//debounces interrupts
...
void setup() {
  ...
  attachInterrupt(digitalPinToInterrupt(ENCODER_BUTTON), key_detectO, CHANGE);
  pinMode(ENCODER_A, INPUT);
  attachInterrupt(digitalPinToInterrupt(ENCODER_A), key_detectA, FALLING);
